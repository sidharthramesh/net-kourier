...
go test -race -count=1 -timeout=20m -tags=e2e ./test/extauthz/grpc/... \
  --ingressendpoint="${IPS[0]}" \
  --ingressClass=kourier.ingress.networking.knative.dev \
  --cluster-suffix="$CLUSTER_SUFFIX"
...
echo ">> Running ExtAuthz tests"
KOURIER_EXTAUTHZ_PACKASBYTES=1 go test -race -count=1 -timeout=20m -tags=e2e ./test/extauthz/grpc/... \
  --ingressendpoint="${IPS[0]}" \
  --ingressClass=kourier.ingress.networking.knative.dev \
  --cluster-suffix="$CLUSTER_SUFFIX"

echo ">> Unset ExtAuthz gRPC with pack as bytes option"
...
echo ">> Running ExtAuthz tests"
go test -race -count=1 -timeout=20m -tags=e2e ./test/extauthz/http/... \
  --ingressendpoint="${IPS[0]}" \
  --ingressClass=kourier.ingress.networking.knative.dev \
  --cluster-suffix="$CLUSTER_SUFFIX"
